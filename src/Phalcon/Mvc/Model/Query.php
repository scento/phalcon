	...
	*/
					...
					if($relation === false) {
						...
						if(is_array($relations) === true) {
							...
							if($numberRelations !== 1) {
								throw new Exception("There is more than one relation between models '".$modelName.
									"' and '".$joinModel.'", the join must be done using an alias, when preparing: '.$this->_phql);
							}
	...
	protected function _prepareSelect()
	{
		...
		foreach($selectedModels as $selectedModel) {
			...
			$model = $this->_manager->load(
				(isset($qualifiedName['ns-alias']) === true ? 
					$this->_manager->getNamespaceAlias($qualifiedName['ns-alias']).'\\'.$modelName :
					$modelName)
				);
	...
	protected function _prepareInsert()
	{
		...
		foreach($ast['values'] as $exprValue) {
			...
			$exprValues[] = array('type' => $exprValue['type'], 
		...
		if(isset($ast['fields']) === true) {
			...
			foreach($ast['fields'] as $field) {
				...
				if($this->_metaData->hasAttribute($model, $field['name']) === false) {
					throw new Exception("The model '".$modelName."' doesn't have the attribute '".
	...
	protected function _executeSelect(array $intermediate, array $bindParams, array $bindTypes)
	{
		...
		return new Simple($simpleColumnMap, $resultObject, 
			($result->numRows($result) > 0 ? $result : false), $this->_cache, $isKeepingSnapshots);
	}
